...
export const getUserTransSummaryService = async(userId: Types.ObjectId) => {
  try {
...
export const getCategoryTotalService = async(userId: Types.ObjectId) => {
  try {
    const user = await User.findById(userId);
    if (!user) {
      throw new ApiError(404, "Please login to get the summary");
...
export const getMonthlyTotalService = async(userId: Types.ObjectId) => {
  try {
    const user = await User.findById(userId);
    if (!user) {
      throw new ApiError(404, "Please login to get the summary");
...
export const getRecentTransService = async(userId: Types.ObjectId) => {
  try {
    const user = await User.findById(userId);
    if (!user) {
      throw new ApiError(404, "Please login to get the summary");
